...
from ...utils.image_utils import to_hwc, pixels_to_pil, is_image
...
def get_image_info(obj: Dict[str, Any], *, config: TrainingConfig) -> Image.Image:
    image = None
...
def to_record(obj: Dict[str, Any], *, config: TrainingConfig, source: str) -> Record:
...
def read_jsonl(path: str, config: TrainingConfig) -> Iterator[Record]:
...
def read_csv(path:str, config: TrainingConfig) -> Iterator[Record]:
...
def read_folder(path: str, config: TrainingConfig) -> Iterator[Record]:
...
class ImageDataset(Dataset):
    def __init__(self, records: List[Record], transform, config: TrainingConfig):
    ...
    def __getitem__(self, idx):
        ...
        text = r.text
        meta = r.meta
        return {"image": image, "label": label, "text": text, "meta": meta}
...
def create_dataloader(config: TrainingConfig, 
                      *,
